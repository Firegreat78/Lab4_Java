...
{
    // Максимально возможный модуль числа
    ...
    {
        if (sign == 0 || str.isEmpty())
        {
            this.sign = 0;
            this.str = "";
            return;
        }
        this.str = StripLeadingZeros(str).length() > MAX_ABS.length() ? MAX_ABS : StripLeadingZeros(str);
        this.sign = this.str.equals("0") ? 0 : Integer.signum(sign);
    }
    ...
    private static String UnsignedMultiply(String a, String b)
    {
        ...
        if (shorter.length() == 1)
        {
            ...
            while (true)
            {
                ...
                if (res == 0 && index < 0) return sb.reverse().toString();
                carry = res / 10;
                sb.append(res % 10);
                index--;
            }
        ...
        else
        {
            ...
            return UnsignedAdd(UnsignedAdd(AC + "0".repeat(n << 1), AD_PLUS_BC + "0".repeat(n)), BD);
        }
    }
